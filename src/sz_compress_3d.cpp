...
// adjust prediction and quantization for sample data
// input 8x8x8 block data
// encode the inner 7x7x7 block
template<typename T>
size_t
prediction_and_quantization_3d_samples(const T * data, const DSize_3d& size, const meanInfo<T>& mean_info, double precision,
	int capacity, int intv_radius, unsigned char * indicator, int * type, 
	int * reg_params_type, float *& reg_unpredictable_data_pos, T *& unpredictable_data_pos){
	const float noise = precision * LorenzeNoise3d;
	int * type_pos = type;
	...
	for(int i=0; i<RegCoeffNum3d-1; i++) 
		reg_precisions[i] = rel_param_err / size.block_size;
	reg_precisions[RegCoeffNum3d - 1] = rel_param_err;
	// change (size.block_size+1) to size.block_size since no padding is needed
	T * pred_buffer = (T *) malloc(size.block_size*size.block_size*size.block_size*sizeof(T));
	memset(pred_buffer, 0, size.block_size*size.block_size*size.block_size*sizeof(T));
	size_t reg_count = 0;
	int capacity_lorenzo = mean_info.use_mean ? capacity - 2 : capacity;
	...
	for(size_t i=0; i<size.num_x; i++){
		...
		for(size_t j=0; j<size.num_y; j++){
			...
			for(size_t k=0; k<size.num_z; k++){
				...
				int min_size = size.block_size;
				// shift start point
				const T * cur_data_pos = z_data_pos + size.dim0_offset + size.dim1_offset + 1;
				compute_regression_coeffcients_3d(cur_data_pos, size_x - 1, size_y - 1, size_z - 1, size.dim0_offset, size.dim1_offset, reg_params_pos);
				*indicator_pos = sz_blockwise_selection_3d(cur_data_pos, mean_info, size.dim0_offset, size.dim1_offset, min_size, noise, reg_params_pos);
				if(*indicator_pos){
					// regression
					// reduce block size
					// because we want to evaluate the 7x7x7 block inside 8x8x8 to get rid of boundary points
					compress_regression_coefficient_3d(reg_precisions, reg_params_pos, reg_params_type_pos, reg_unpredictable_data_pos);
					block_pred_and_quant_regression_3d(cur_data_pos, reg_params_pos, precision, capacity, intv_radius, 
						size_x - 1, size_y - 1, size_z - 1, size.dim0_offset, size.dim1_offset, type_pos, unpredictable_data_pos);
					reg_count ++;
					reg_params_pos += RegCoeffNum3d;
					reg_params_type_pos += RegCoeffNum3d;
				}
				else{
					// fill pred_buffer for Lorenzo
					{
						T * buffer_pos = NULL;
						const T * block_pos = NULL;
						// i=0
						for(int j=0; j<size_y; j++){
							buffer_pos = pred_buffer + j*buffer_dim1_offset;
							block_pos = z_data_pos + j*size.dim1_offset;
							memcpy(buffer_pos, block_pos, size_z*sizeof(T));
						}
						// j=0
						for(int i=1; i<size_x; i++){
							buffer_pos = pred_buffer + i*buffer_dim0_offset;
							block_pos = z_data_pos + i*size.dim0_offset;
							memcpy(buffer_pos, block_pos, size_z*sizeof(T));
						}
						// k=0
						for(int i=1; i<size_x; i++){
					...
					// Lorenzo
					// change buffer size to block size
					// reduce block size
					// because we want to evaluate the 7x7x7 block inside 8x8x8 to get rid of boundary points
					block_pred_and_quant_lorenzo_3d(mean_info, cur_data_pos, pred_buffer, precision, capacity_lorenzo, intv_radius, 
						size_x - 1, size_y - 1, size_z - 1, buffer_dim0_offset, buffer_dim1_offset, size.dim0_offset, size.dim1_offset, type_pos, unpredictable_data_pos);
				}
				indicator_pos ++;
				z_data_pos += size_z;
			}
...
template<typename T>
unsigned char *
sz_compress_3d(const T * data, size_t r1, size_t r2, size_t r3, double precision, size_t& compressed_size, int BSIZE3d, char pred_option){
	...
	switch(pred_option){
		case BLOCK_INDEPENDANT: 
			reg_count = prediction_and_quantization_3d(data, size, mean_info, precision, capacity, intv_radius, indicator, type, reg_params_type, reg_unpredictable_data_pos, unpredictable_data_pos);
			break;
		case BLOCK_DEPENDANT:
			reg_count = prediction_and_quantization_3d_with_border_prediction(data, size, mean_info, precision, capacity, intv_radius, indicator, type, reg_params_type, reg_unpredictable_data_pos, unpredictable_data_pos);
			break;
		case BLOCK_SAMPLE:
			reg_count = prediction_and_quantization_3d_samples(data, size, mean_info, precision, capacity, intv_radius, indicator, type, reg_params_type, reg_unpredictable_data_pos, unpredictable_data_pos);
			break;
		default:
			reg_count = prediction_and_quantization_3d(data, size, mean_info, precision, capacity, intv_radius, indicator, type, reg_params_type, reg_unpredictable_data_pos, unpredictable_data_pos);
			break;
	...
	write_variable_to_dst(compressed_pos, pred_option);
	...
	if(pred_option == BLOCK_SAMPLE) Huffman_encode_tree_and_data(2*capacity, type, size.num_blocks * (size.block_size - 1) * (size.block_size - 1) * (size.block_size - 1), compressed_pos);
	else Huffman_encode_tree_and_data(2*capacity, type, size.num_elements, compressed_pos);
...
template 
unsigned char * 
sz_compress_3d<float>(const float * data, size_t r1, size_t r2, size_t r3, double precision, size_t& compressed_size, int BSIZE3d, char pred_option);
